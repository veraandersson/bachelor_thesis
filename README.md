...
* **Model:** An augmented **SEIR (Susceptible-Exposed-Infectious-Recovered)** compartmental model where the usual recovered state is split into three states: recovered, long-term ill, and deceased.
...
* **Mathematics:** The final epidemic sizes for each demographic group were computed by numerically solving balance equations.
* **Results:** The optimal vaccination scheme depends on the end goal. The vaccination scheme prioritizing younger individuals reduces transmission the most, while the scheme prioritizing vulnerable indviduals protects risk groups the most. However, vaccinating individuals with a higher risk of early infection sufficiently reduces both transmission and protects risk groups.
